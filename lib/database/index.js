  ...
  constructor() {
    if (process.env.DBTYPE != undefined && process.env.DBTYPE.toUpperCase() == 'LOCAL') {
      ...
      this.getStats = function () {
        ...
        return new Promise(function (resolve, reject) {
          bigquery.all("SELECT CASE mutant_indicator WHEN 1 THEN 'count_mutant_dna' ELSE 'count_human_dna' END indicator, COUNT(1) count FROM mutant_data.dna_sequences GROUP BY mutant_indicator", [], (err, rows) => {
            if (err) {
              throw err;
            }
            var obj = {};
